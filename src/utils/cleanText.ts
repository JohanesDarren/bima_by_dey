/**
 * Pembersih teks yang datang dari AI.
 *
 * Dua lapis:
 * 1. Penanda markdown dibuang (AI sering menulis **tebal**, "# judul", "- butir").
 * 2. Keterangan dalam tanda kurung yang tidak berguna dibuang.
 *    Contoh nyata dari lapangan: "150 g tepung sorgum (55% dari tepung)" — angka
 *    persen seperti itu cuma menyesatkan pembaca dan tidak dipakai memasak.
 *
 * Dipakai untuk kolom pendek: bahan, judul/isi langkah, deskripsi menu.
 * Untuk jawaban chat dipakai cleanAssistantAnswer (potong blok "Catatan
 * Verifikasi" yang ditempel server, buang markdown & bungkus kurung kurawal).
 */
...
/** Panjang maksimal keterangan dalam kurung yang masih dianggap berguna.
 *  Di atas ini hampir pasti penjelasan panjang (mis. "(yang sudah matang
 *  sekali supaya manis alami)") yang tidak dipakai saat memasak. */
...
interface CleanOptions {
   ...
   *   seharusnya cuma nama + jumlah + satuan ("170 g tepung sorgum", bukan
   *   "170 g tepung sorgum (±55% dari tepung)").
   */
  notes?: 'junk' | 'all';
}
...
/**
 * Bersihkan jawaban Chef untuk ditampilkan & dibacakan:
 * - penanda markdown dibuang,
 * - blok "Catatan Verifikasi" + skor dipotong; itu laporan mutu RAG, bukan
 *   jawaban koki, dan bikin pembaca awam bingung ("resep ini belum layak"),
 * - bungkus kurung kurawal / garis pemisah sisa pembungkus dibuang.
 */
export function cleanAssistantAnswer(text: string): string {
  let s = stripMarkdown(text);

  const cut = s.search(VERIFICATION_BLOCK);
